...
{
    unsigned long flags;
    /* 加自旋锁并关闭中断，防止在检查和修改devled.statu时被打断 */
    spin_lock_irqsave(&devled._slcok ,flags);
    if(devled.statu){
        /* 解锁并恢复之前的中断状态 */
        spin_unlock_irqrestore(&devled._slcok ,flags);
        pr_notice("\r\n");
        pr_notice("dev bussy !!!\r\n");
        return -EBUSY;
    }
...
static int _release(struct inode *pinode,struct file *filp)
{
    ...
    __udev *_udev = filp->private_data;
    /* 上锁，保护变量statu */
    spin_lock_irqsave(&_udev->_slcok ,flags);
    if(_udev->statu){
        _udev->statu = 0;
    }
    /* 释放自旋锁 */
    spin_unlock_irqrestore(&_udev->_slcok ,flags);
    return 0;
}
...
{   
    /* 初始化自旋锁 */
    spin_lock_init(&devled._slcok);
    /* 初始化设备状态 */
    devled.statu = 0;
    pr_notice("spinlock init successd!!!\r\n");
}
/* 在设备树里查找对应硬件节点 */
