...
static int _open(struct inode *pinode,struct file *filp)
{
    /* 获取互斥体 返回值为0，成功返回信号量
       进入休眠状态的进程可以被信号打断*/
    if(mutex_lock_interruptible(&devled.mtx)){
        pr_err("get mutex error\rn");
        return -EINTR;
    }
...
{   
    /* 初始化互斥体 */
    mutex_init(&devled.mtx);
    pr_notice("mutex init successd!!!\r\n");
}
/* 在设备树里查找对应硬件节点 */
