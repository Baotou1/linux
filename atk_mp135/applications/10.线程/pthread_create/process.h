...
/**
 * @macro  PROC_INFO_LOG
 * @brief  记录进程信息的日志宏，格式为单行显示
 *
 * @param __proc  指向进程结构体的指针，包含进程相关信息
 * @param __act   操作动作名称（如 INIT、EXIT），用于日志标签
 *
 * @details
 * 该宏调用 _log_write 函数，打印格式化的进程信息日志，输出格式如下：
 *   [PROCESS][动作] info: name=进程名, pid=进程ID, pgid=进程组ID, ppid=父进程ID, sid=会话ID
 *
 * 使用 do { ... } while(0) 结构保证宏的语法安全，适合在各种语句块中调用。
 */                                    
#define PROC_INFO_LOG(__proc, __act)\
                                    do{\
                                        _log_write("[PROCESS][" #__act "]", "info: name->%s, pid->%d, pgid->%d, ppid->%d, sid->%d", \
                                            (__proc)->__name, \
                                            (__proc)->__pid, \
                                            (__proc)->__pgid, \
                                            (__proc)->__ppid, \
