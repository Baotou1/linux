 ...
 * @param[in]  __pid  待查询的进程 PID，通常为目标进程的进程号
 *
 * @retval int 成功返回 0；失败返回 -1，并打印错误信息（errno 指示原因）
 *
 * @details
 *  本函数封装了 getpgid() 系统调用，用于查询某个进程的进程组 ID。
 *  若传入无效参数或系统调用失败，则返回 -1。
 *
 * @note
 *  - `__pgid` 指针不能为空，调用前需为其分配空间；
 *  - 若需查询当前进程的 PGID，可传入 getpid() 作为 __pid；
 *  - getpgid() 失败时 errno 通常为 ESRCH（进程不存在）或 EPERM（无权限）。
 */
...
/**
 * @name    __proc_setpgid
 * @brief   设置指定进程的进程组 ID（PGID）
 *
 * @param[in] __pgid  目标进程组 ID，指定该进程将被加入的进程组
 * @param[in] __pid   目标进程 ID，表示要设置的进程
 *
 * @retval int 成功返回 0；失败返回 -1，并打印错误信息（errno 指示原因）
 *
 * @details
 *  本函数封装了 setpgid() 系统调用，用于改变指定进程的进程组 ID。
 *  可用于创建新的进程组，或将进程加入已有进程组。
 *
 * @note
 *  - 调用者需具备对指定 PID 的权限；
 *  - 若 __pid 为 0，表示当前进程；若 __pgid 为 0，则使用 __pid 作为 PGID；
 *  - 通常需在目标进程尚未执行 exec 或未成为组长前调用；
 *  - 常见失败原因包括权限不足（EPERM）、进程不存在（ESRCH）或进程状态不允许更改（EINVAL）。
 */
...
/**
 * @name    __proc_getsid
 * @brief   获取指定进程的会话 ID（SID）
 *
 * @param[out] __sid  用于存放获取到的会话 ID，不能为空
 * @param[in]  __pid  目标进程的 PID；若为 0，表示当前调用进程
 *
 * @retval int 成功返回 0；失败返回 -1，并打印错误信息（errno 指示原因）
 *
 * @details
 *  本函数封装了 getsid() 系统调用，用于查询指定进程所属的会话 ID（SID）。
 *  会话是进程管理的高级抽象单位，多个进程可以属于同一会话，通常与控制终端相关联。
 *
 * @note
 *  - `__sid` 不可为 NULL，否则直接返回 -1；
 *  - 若 `__pid` 指定的进程不存在，或者调用者权限不足，则返回 -1，并设置 errno：
 *      - ESRCH：指定的进程不存在；
 *      - EPERM：没有足够权限获取目标进程信息；
 *  - 返回的 SID 通常等于会话领导进程（Session Leader）的 PID。
 */
...
/**
 * @name    __proc_setsid
 * @brief   创建新的会话（session），使当前进程成为会话首领，并返回会话 ID
 *
 * @param[out] __sid  用于接收新创建的会话 ID（Session ID），不能为空
 *
 * @retval int
 *      成功返回 0，且 *__sid 被赋值为新会话的 ID；
 *      失败返回 -1，并打印错误信息（errno 指示具体原因）
 *
 * @details
 *  本函数封装了 setsid() 系统调用，用于将当前进程设置为新会话的会话首领（Session Leader），
 *  同时脱离其原有进程组和控制终端。该操作通常用于守护进程（daemon）创建流程的第一步。
 *
 *  为避免调用 setsid() 失败（EPERM），本函数在调用前主动检查当前进程是否为其进程组的组长，
 *  若是，则直接返回错误并输出提示信息，防止无效调用。
 *
 * @note
 *  - `__sid` 不可为 NULL，否则函数直接返回 -1，且不设置 errno；
 *  - 若当前进程是其进程组组长（即 PGID == PID），setsid() 将失败；
 *      - 此情况下本函数会提前返回，避免调用 setsid()；
 *  - 成功调用 setsid() 后：
 *      - 当前进程成为新会话的会话首领；
 *      - 返回的新会话 ID 等于当前进程的 PID；
 *      - 当前进程脱离原控制终端（若有）；
 *  - 守护进程通常在 setsid() 成功之后继续执行如下操作：
 *      - 设置文件权限掩码（umask）；
 *      - 切换工作目录（chdir）；
 *      - 关闭标准输入/输出/错误描述符；
 *
 * @see setsid(2), getsid(2), getpid(2), getpgid(2)
 */
int __proc_setsid(pid_t *__sid)
{
    ...
    pid_t __pgid ,__pid;
    __proc_getpid(&__pid);
    if(__proc_getpgid(&__pgid ,__pid) == -1)
    {
        return -1;
    }   
    if(__pgid == __pid)
    {
        fprintf(stderr, "[ERROR]: setsid() denied: current process is group leader (PID = %d, PGID = %d)\n", __pid, __pgid);
        return -1;
    } 
...
/**
 * @func    __proc_execve
 * @brief   执行 execve 系统调用，加载并执行新程序。
 *
 * @param[in] __exec  指向执行信息结构体 __exec_t 的指针，包含路径、参数和环境变量。
 *
 * @retval int        成功不返回（调用成功时当前进程会被替换）；  
 *                    若执行失败，返回 -1，并设置 errno 指示错误原因。
 *
 * @details
 *  本函数封装了 execve() 系统调用，用于在当前进程上下文中加载并执行另一个程序。
 *  - 若 __exec 或其中的成员为 NULL，则直接返回 -1；
 *  - execve 成功调用不会返回，当前进程映像将被新程序替换；
 *  - 若 execve 执行失败，返回 -1，调用方可通过 errno 获取详细错误信息。
 *
 * @note
 *  - 调用本函数前应确保 __argv 和 __envp 均以 NULL 结尾；
 *  - 建议在子进程中调用该函数，避免影响父进程流程；
 *  - 若需携带环境变量执行外部程序，envp 可使用 environ 或自定义构造。
 */
int __proc_execve(__exec_t *__exec)
{
    if(__exec == NULL || __exec->__argv == NULL || __exec->__envp == NULL || __exec->__path == NULL)
        return -1;

    return execve(__exec->__path ,__exec->__argv ,__exec->__envp);
}

/**
 * @func    __proc_execv
 * @brief   执行 execv 系统调用，加载并执行新程序（不传递自定义环境变量）。
 *
 * @param[in] __exec  指向执行信息结构体 __exec_t 的指针，包含路径和参数。
 *
 * @retval int        成功不返回（调用成功时当前进程会被替换）；  
 *                    若执行失败，返回 -1，并设置 errno 指示错误原因。
 *
 * @details
 *  本函数封装了 execv() 系统调用，用于在当前进程上下文中加载并执行另一个程序。
 *  - 若 __exec 为 NULL，或其成员 __path 或 __argv 为 NULL，则直接返回 -1；
 *  - execv 成功调用不会返回，当前进程映像将被新程序替换；
 *  - 若 execv 执行失败，返回 -1，调用方可通过 errno 获取详细错误信息。
 *
 * @note
 *  - __argv 参数必须以 NULL 结尾；
 *  - 本函数使用当前进程的默认环境变量（即 environ）；
 *  - 建议在子进程中调用该函数，以避免主流程被替换；
 *  - 若需要传递自定义环境变量，请使用 __proc_execve 替代。
 */
int __proc_execv(__exec_t *__exec)
{
    if(__exec == NULL || __exec->__argv == NULL || __exec->__path == NULL)
        return -1;

    return execv(__exec->__path ,__exec->__argv);
}
...
/**
 * @func    __proc_system
 * @brief   封装 system()，用于执行 shell 命令或检测 shell 可用性。
 *
 * @param[in] __command  shell 命令字符串，或为 NULL 检测 shell 是否可用。
 *
 * @retval int
 *  - 若 __command 为 NULL：
 *      - 返回 1 表示 shell 可用；
 *      - 返回 0 表示 shell 不可用；
 *  - 若 __command 非 NULL：
 *      - 返回 system(__command) 的原始返回值（非负表示成功，-1 表示调用失败）。
 */
