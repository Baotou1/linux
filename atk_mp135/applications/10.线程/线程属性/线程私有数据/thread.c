...
/**
 * @brief 创建线程特有数据（TLS）键，并指定销毁函数
 *
 * @details
 * 此函数封装了 pthread_key_create，用于为线程分配一个线程私有数据键（pthread_key_t），
 * 可用于在线程中存储每个线程独立的数据（如上下文、缓存等）。
 * 每个线程都可以通过该 key 设置和获取属于自己的数据副本，互不干扰。
 *
 * 如果为 __destructor 指针提供了非 NULL 的析构函数，则在线程退出时，
 * 系统会自动调用该函数并传入线程绑定的私有值指针，实现资源释放。
 *
 * @param[in,out] __tls  指向线程 TLS 结构体 __thd_tls_t 的指针，不能为空，且其成员 __destructor 应为非 NULL。
 *                       创建成功后，__key 字段将被初始化。
 *
 * @return
 *   -  0 ：创建成功；
 *   - -1 ：参数非法（__tls 或 __destructor 为 NULL）；
 *   - >0 ：pthread_key_create 返回的错误码（如 ENOMEM、EAGAIN）。
 *
 * @note
 * - 该函数应在线程创建前调用，建议仅调用一次，可结合 pthread_once 使用；
 * - 每个 __key 对应一组线程私有数据值，数据由线程自己设置并访问；
 * - 使用完毕后，建议调用 pthread_key_delete 删除 key；
 * - 创建的键在所有线程中通用，但每个线程维护自己对应的值；
 * - 析构函数不能调用 pthread_setspecific 修改值本身，否则可能造成死循环。
 *
 * @see pthread_key_create(3), pthread_setspecific(3), pthread_getspecific(3), pthread_key_delete(3)
 */
int __thread_key_create(__thd_tls_t *__tls)
{
    if(__tls == NULL || __tls->__destructor == NULL)
        return -1;

    int __ret = pthread_key_create(&__tls->__key ,__tls->__destructor);
    if(__ret != 0)
    {
        return __ret;
    }
...
/**
 * @func   __thread_key_setspecific
 * @brief  设置当前线程的线程局部存储（TLS）数据
 *
 * @details
 *  本函数是对 pthread_setspecific 的封装，用于将指定线程私有数据结构体 __tls
 *  中的键 (__key) 绑定到当前线程的特定值 (__value)。
 *  该值仅对调用线程可见，不同线程对同一键可绑定不同的值，实现线程数据隔离。
 *
 *  通常，线程在执行期间会为自己绑定特定的上下文数据、缓存或状态信息，避免
 *  与其他线程数据冲突。
 *
 * @param[in,out] __tls    指向线程 TLS 结构体的指针，不能为空。
 *                         该结构体中存储了线程私有键 (__key)。
 * @param[in]     __value  绑定到当前线程上的私有数据指针，不能为空。
 *
 * @return
 *   - 0   : 绑定成功
 *   - -1  : 参数错误 (__tls 或 __value 为 NULL)
 *   - >0  : pthread_setspecific 返回的错误码（例如 ENOMEM）
 *
 * @note
 *  - 同一线程对同一键调用多次本函数，后一次绑定会覆盖之前绑定的值；
 *  - 绑定的值会在对应线程退出时调用注册的析构函数释放（若注册了析构函数）；
 *  - 调用本函数前应确保键已成功创建（通过 pthread_key_create 或封装接口）；
 *  - 建议绑定的数据有统一的申请与释放管理，避免内存泄漏或悬空指针。
 *
 * @see pthread_setspecific(3), pthread_key_create(3), pthread_getspecific(3), pthread_key_delete(3)
 */
int __thread_key_setspecific(__thd_tls_t *__tls ,const void *__value)
{
    if(__tls == NULL || __value == NULL)
        return -1;

    int __ret = pthread_setspecific(__tls->__key ,__value);
    if(__ret != 0)
    {
        return __ret;
    }
...
/**
 * @func   __thread_key_getspecific
 * @brief  获取当前线程对应的线程特有数据（TLS）
 *
 * @details
 * 本函数封装了 pthread_getspecific，用于从线程局部存储中获取由指定键关联的线程特有数据。
 * 返回的数据指针仅对当前线程有效，且需调用者自行进行类型转换。
 *
 * @param[in] __tls  指向线程特有数据结构体 __thd_tls_t 的指针，不能为空。
 *                   该结构体包含已创建的 pthread_key_t 键。
 *
 * @return
 *   - 非 NULL ：返回当前线程绑定的特有数据指针；
 *   - NULL    ：表示未设置数据或参数非法。
 *
 * @note
 * - 该函数只获取数据，不修改 __tls 结构体中的 __value 字段；
 * - 若线程未通过 pthread_setspecific 设置数据，则返回 NULL；
 * - 返回的数据应由调用线程在设置时确保合法性，避免类型不匹配；
 * - TLS 数据的生命周期需用户控制，若注册了析构函数，线程退出时将自动释放。
 *
 * @see pthread_getspecific(3), pthread_setspecific(3), pthread_key_create(3)
 */
void *__thread_key_getspecific(__thd_tls_t *__tls)
{
    if(__tls == NULL)
        return NULL;

    return  pthread_getspecific(__tls->__key);
}

/**
 * @func   __thread_key_delete
 * @brief  删除线程特有数据键（TLS key）
 *
 * @details
 * 本函数封装了 pthread_key_delete，用于销毁线程特有数据键。
 * 删除键后，相关的线程特有数据将不再可访问，键不可复用。
 *
 * @param[in] __tls  指向线程特有数据结构体 __thd_tls_t 的指针，不能为空。
 *                   该结构体包含要删除的 pthread_key_t 键。
 *
 * @return
 *   -  0  ：删除成功；
 *   - -1  ：参数非法（如 __tls 为 NULL）；
 *   - >0  ：pthread_key_delete 返回的错误码（如 EINVAL 表示键无效）。
 *
 * @note
 * - 删除键不会调用已绑定数据的析构函数；
 * - 删除键后，相关线程不能再使用该键访问 TLS 数据；
 * - 仅当不再需要该键时调用此函数。
 *
 * @see pthread_key_delete(3), pthread_key_create(3)
 */
int __thread_key_delete(__thd_tls_t *__tls)
{
    if(__tls == NULL)
        return -1;

    int __ret = pthread_key_delete(__tls->__key);
    if(__ret != 0)
    {
        return __ret;
    }
