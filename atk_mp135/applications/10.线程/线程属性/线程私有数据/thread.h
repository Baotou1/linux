...
/**
 * @struct __thd_tls_t
 * @brief  线程特有数据（Thread-Specific Data, TSD）键结构体封装
 *
 * @details
 * 本结构体封装了 POSIX 线程特有数据机制中涉及的键对象 pthread_key_t，
 * 以及用户自定义的析构函数和当前线程关联的值指针。
 * 适用于管理每个线程拥有独立副本的数据，用于避免线程间数据共享造成的竞争。
 *
 * 成员说明：
 * - __key        : pthread_key_t 类型的键，由 pthread_key_create 创建，用于唯一标识线程私有数据；
 * - __destructor : 线程退出时自动调用的析构函数指针，释放通过 pthread_setspecific 设置的资源；
 * - __value      : 当前线程绑定到该 key 的线程私有数据指针，可通过 pthread_getspecific 获取；
 *
 * 使用方式示例：
 *  1. 创建键：pthread_key_create(&__key, __destructor);
 *  2. 设置值：pthread_setspecific(__key, __value);
 *  3. 获取值：pthread_getspecific(__key);
 *  4. 删除键：pthread_key_delete(__key);
 *
 * @note
 * - __value 是线程私有的，每个线程调用 pthread_getspecific(__key) 得到的值互不干扰；
 * - 若指定了 __destructor，线程退出时会自动对绑定值调用此函数；
 * - 本结构体仅为封装，__value 的更新应通过 pthread_setspecific，而非直接赋值。
 */
typedef struct
{
    pthread_key_t __key;               ///< 线程私有数据键，唯一标识一个线程特有数据项
    void (*__destructor)(void*);       ///< 线程退出时的析构函数，用于释放绑定值
    __thd_once_t __once;
}__thd_tls_t;
