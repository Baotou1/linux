...
/**
 * @name    _time_get_local_str
 * @brief   将指定时间格式化为字符串（本地时间，格式："YYYY-MM-DD HH:MM:SS"）
 *
 * @param[in]  __timer  指向 time_t 类型的时间戳（秒）
 * @param[out] __buf    用于保存格式化后的时间字符串，建议大小不少于 20 字节
 *
 * @return     成功写入到 buf 中的字符数（不包括 \0 结尾符）
 */
size_t _time_get_local_str(time_t *__timer, char *__buf)
{
    if(__timer == NULL || __buf == NULL)
        return 0;

    struct tm _tm;
    // 获取当前时间（单位：秒），并写入传入的 __timer 指针中
    *__timer = time(NULL);

    // 将 time_t 类型的时间转换为本地时间（struct tm），结果存入 _tm 结构体
    localtime_r(__timer, &_tm);

    // 使用格式 "YYYY-MM-DD HH:MM:SS" 格式化时间，写入 __buf
    // 返回实际写入的字符数（不包括末尾的 '\0'）
    return strftime(__buf, 100, "%Y-%m-%d %H:%M:%S", &_tm);
}

/**
 * @func    _time_get_timestamp
 * @brief   获取当前高精度单调时间戳（单位：秒）
 *
 * @return
 *  - 返回从系统启动到当前的时间（以秒为单位，包含小数部分）；
 *  - 若调用 clock_gettime 失败，返回值不可靠（但在现代系统中几乎不会失败）。
 *
 * @details
 *  该函数通过调用 clock_gettime(CLOCK_MONOTONIC) 获取单调递增的系统时间，
 *  与系统时间（如 time(NULL)）不同，该时间不会因系统时钟调整而跳变。
 *
 *  适用于：
 *  - 性能测量（如代码段执行耗时）；
 *  - 间隔判断、超时控制；
 ...
 *
 * @note
 *  - 精度可达纳秒（通常在 1~10 毫秒范围，取决于系统实现）；
 *  - 若需要获取真实时间戳（如“1970 年以来的秒数”），应使用 time() 或 gettimeofday()；
 *  - 若对时间差值操作可接受精度略低，可用 time(NULL) 替代。
 */
double _time_get_timestamp(void)
{
    struct timespec __ts;
     // 获取当前单调时钟时间（不受系统时间变化影响）
    // CLOCK_MONOTONIC 表示系统启动到当前的持续时间
    clock_gettime(CLOCK_MONOTONIC, &__ts);

    // 返回当前时间戳（单位：秒，带小数），精度约为纳秒
    // ts.tv_sec 是秒数，ts.tv_nsec 是纳秒数
    // 除以 1e9 将纳秒转换为秒后与整数秒相加，形成 double 精度的时间戳
    return __ts.tv_sec + __ts.tv_nsec / 1e9;
}
...
int _file_get_properties(char *__pathname ,struct __file_stat *fst)
{
    ...
    _time_get_local_str(&fst->st.st_atim.tv_sec ,fst->atim);
    _time_get_local_str(&fst->st.st_mtim.tv_sec ,fst->mtim);
    _time_get_local_str(&fst->st.st_ctim.tv_sec ,fst->ctim);
 ...
 * @brief 删除指定路径的文件或空目录。
 * 
 * @param __pathname 要删除的文件或空目录路径
 * 
 * @return 成功返回 FILE_EOK，失败返回 -FILE_ERROR
 * 
 * @details
 *  封装标准库函数 remove()，用于删除文件或空目录：
 *   - 若路径不存在，则视为“已删除”成功；
 *   - 否则尝试调用 remove() 删除；
 *   - 删除失败时输出错误信息并返回 -FILE_ERROR。
 * 
 * @note
 *  - remove() 可以删除普通文件或 **空目录**；
 *  - 若需要递归删除目录，请使用 `unlink()` + `rmdir()` + 遍历实现；
 *  - 若路径为符号链接，仅删除链接本身，而不影响链接目标。
 */
int _file_remove(const char *__pathname)
{
    if(__pathname == NULL)
        return -FILE_ERROR;
 ...
 * @brief 重命名文件或目录。
 * 
 * @param __oldpath 原路径名（需存在）
 * @param __newpath 新路径名（目标路径）
 * 
 * @return 成功返回 FILE_EOK，失败返回 -FILE_ERROR
 * 
 * @details
 *  封装标准系统调用 rename()，实现文件或目录的重命名或移动操作：
 *   - 当 __newpath 所在位置已存在同名文件，会被替换（依据系统实现）；
 *   - 支持对目录的重命名，但移动跨设备（mount point）时会失败；
 *   - 出错时调用 PRINT_ERROR 输出 errno。
 * 
 * @note
 *  - 不支持跨挂载点移动；
 *  - 如需更复杂的移动操作，可结合 `renameat()`、`link()` 和 `unlink()` 使用。
 */
int _file_rename(const char *__oldpath, const char *__newpath)
{
    if(__oldpath == NULL || __newpath == NULL)
        return -FILE_ERROR;

    if(rename(__oldpath, __newpath) == -1){
        PRINT_ERROR();
        return -FILE_ERROR;
    }
 ...
 * @brief   关闭文件并释放关联资源。
 *
 * 该函数用于关闭 _file_t 结构体对应的文件描述符，并释放结构体中分配的动态内存资源，
 * 包括文件路径名字符串和数据缓冲区等。注意，结构体成员 `pf->pw` 通常由 `getpwuid()` 返回，
 * 指向的是静态内存，不应调用 `free()` 释放。
 *
 * 传入的指针 `pf` 可以为 NULL，函数内部将做空指针检查，因此调用者无需在外部判断。
 *
 * @param[in] pf 指向 _file_t 结构体的指针，可为 NULL。
 *
 * @return  无返回值。
 */
 ...
 *
 * @param[in,out] __pf 指向 _file_t 结构体的指针，不能为空。
 * @param[in] __fg 文件打开标志（如 O_RDONLY、O_WRONLY | O_CREAT 等）。
 * @param[in] __md 文件权限模式（如 0666），仅在创建新文件时有效。
 *
 * @return 成功返回 FILE_EOK（通常为 0），失败返回负错误码。
 */
int _file_open(_file_t *__pf ,int __fg ,mode_t __md)
{
    if(__pf == NULL || __fg < 0)
        return -FILE_ERROR;
    ...
    __pf->fd = open(__pf->__pathname ,__pf->fg ,__md);
    if(__pf->fd == -1)
    {
        PRINT_ERROR();
        return -FILE_ERROR;
    ...
    if(_file_get_properties(__pf->__pathname ,__pf->fst) == -FILE_ERROR)
        return -FILE_ERROR;

    if(_file_get_offset(__pf) == -FILE_ERROR)
        return -FILE_ERROR;

    if(_file_data_init(&__pf->data ,1) == -FILE_ERROR)
        return -FILE_ERROR;

#ifdef PIRNT
    PRINT_FILE_INFO("open" ,__pf);
#endif
 ...
 *
 * @param[in,out] __pfw     指向目标文件结构体 _file_t 的指针。
 * @param[in]     __data    指向要写入的数据缓冲区。
 * @param[in]     __ofs     写入时设置的偏移值。
 * @param[in]     __whence  偏移设置方式（SEEK_SET、SEEK_CUR、SEEK_END）。
 * @param[in]     __len     要写入的数据字节数，必须 > 0。
 *
 * @return 成功返回实际写入的字节数，失败返回 -FILE_ERROR。
 */
int _file_write(_file_t *__pfw ,void *__data ,off_t __ofs ,int __whence ,size_t __len)
{
    if(__pfw == NULL || __data == NULL || __len <= 0 ||
        (__whence != SEEK_CUR && __whence != SEEK_SET && __whence != SEEK_END))
        return -FILE_ERROR;

    if(_file_set_offset(__pfw ,__ofs ,__whence) == -FILE_ERROR)
        return -FILE_ERROR;
#ifdef PRINT
    printf("set %s file write offset: %ld bytes\n" ,__pfw->__pathname ,__pfw->ofs);
#endif

    __pfw->ret = write(__pfw->fd ,__data ,__len);
    if(__pfw->ret == -1){
        PRINT_ERROR();
        return -FILE_ERROR;
    }
    
    if(_file_get_info(__pfw) == -FILE_ERROR)
        return -FILE_ERROR;

#ifdef PRINT
    PRINT_FILE_INFO("write" ,__pfw);
#endif
...
/**
 * @func   _file_pread
 * @brief  从文件中按指定偏移读取数据（不影响文件当前偏移）。
 *
 * @details
 *  使用 pread() 系统调用从文件描述符指定的偏移处读取 `len` 字节数据，
 *  读取内容保存至 `pfr->data` 缓冲区。该操作不会改变文件描述符的当前偏移量。
 *
 *  若读取超出文件末尾，读取长度将被自动裁剪为：`文件大小 - ofs`。
 *  若读取失败、参数非法或元信息更新失败，返回 -FILE_ERROR。
 *
 * @param[in,out] __pfr   指向目标文件结构体 _file_t 的指针，必须已打开。
 * @param[in]     __ofs   起始读取偏移（文件内的位置）。
 * @param[in]     __len   要读取的字节数（> 0）。
 *
 * @retval >0          实际成功读取的字节数。
 * @retval -FILE_ERROR 出现错误（如文件未打开、偏移非法、IO失败等）。
 *
 * @note
 *  - `pfr->data` 将由本函数内部重新分配，请确保调用前已释放旧内存（如有）。
 *  - 本函数会更新 `pfr` 中的元信息（如大小、偏移、权限等）。
 *  - 与 `read()` 不同，`pread()` 是线程安全的，不影响进程共享文件偏移。
 */
int _file_pread(_file_t *__pfr  ,off_t __ofs ,size_t __len)
{
    if(__pfr == NULL || __pfr->data == NULL || __len <= 0)
        return -1;
...
//#define __PRINT_FILEOFS
    ...
    if(_file_get_properties(__pfr->__pathname ,__pfr->fst) == -FILE_ERROR)
        return -FILE_ERROR;

    __len = (__len > (__pfr->fst->st.st_size - __ofs)) ?  
                                         (__pfr->fst->st.st_size - __ofs): __len;

    if(_file_data_init(&__pfr->data ,__len) == -FILE_ERROR)
        return -FILE_ERROR;

    __pfr->ret = pread(__pfr->fd ,__pfr->data ,__len ,__ofs);    
    if(__pfr->ret < 0)
    {
        PRINT_ERROR();
        return -FILE_ERROR;
    ...
    if(_file_get_info(__pfr) == -FILE_ERROR)
        return -FILE_ERROR;

#ifdef PRINT
    PRINT_FILE_INFO("read" ,__pfr);
#endif
 ...
 *
 * @param[in,out] __pf    文件结构体指针，需已打开
 * @param[in]     __cmd   F_GETFL 或 F_SETFL
 * @param[in]     ...     若 __cmd 为 F_SETFL，需传入 int flag，表示待设置标志
 *
 * @retval  FILE_EOK     成功
 ...
 */
int _file_status_fcntl(_file_t *__pf ,int __cmd, ...)
{
    if(__pf == NULL || (__cmd != F_GETFL && __cmd != F_SETFL))
        return -FILE_ERROR;
    ...
    if(__cmd == F_GETFL)
    {
        nfg = fcntl(__pf->fd ,F_GETFL);
    }
    else if(__cmd == F_SETFL)
    {
        va_start(args, __cmd);
        ...
        if(HAS_INVALID_F_SETFL_FLAGS(flag) == 0)
        {
            ...
            if(fcntl(__pf->fd ,F_SETFL ,ofg | flag) == -1){
                nfg = -1;
            }else{
                nfg = ofg | flag;
            }
...
/**
 * @func   _file_flock
 * @brief  对指定文件对象执行加锁或解锁操作
 *
 * @param[in] __pf   指向 _file_t 文件结构体的指针，必须是有效打开的文件
 * @param[in] __opt  锁操作选项，可为以下宏的组合：
 *                     - LOCK_SH：共享锁（读锁）
 *                     - LOCK_EX：独占锁（写锁）
 *                     - LOCK_UN：解锁
 *                     - LOCK_NB：非阻塞方式获取锁（可与 LOCK_SH / LOCK_EX 组合）
 *
 * @retval FILE_EOK     加锁或解锁成功
 * @retval -FILE_ERROR  操作失败，可能是参数无效、fd 非法或锁冲突（若使用 LOCK_NB）
 *
 * @details
 *  本函数对指定文件描述符执行 flock 锁操作，适用于对同一文件在多个进程或线程间进行
 *  同步访问控制。
 *
 * @note
 *  - 使用 LOCK_NB 可避免阻塞等待锁。
 *  - 此函数要求 __pf 中的 fd 是有效的可用文件描述符。
 *  - 锁是“建议性锁”，仅在各进程显式使用 flock 时才有效。
 */
int _file_flock(_file_t *__pf ,int __opt)
{
    /* 参数检查：文件结构体不能为空，且操作选项必须包含有效锁类型 */
    if(__pf == NULL || (__opt & (LOCK_SH | LOCK_EX | LOCK_UN | LOCK_NB)) == 0)
        return -FILE_ERROR;

    /* 调用 flock 进行加锁或解锁操作 */
    if(flock(__pf->fd ,__opt) == -1)
    {
        /* 锁操作失败，打印错误信息 */
        PRINT_ERROR();
        fprintf(stderr, "[ERROR]: Program is already running. Cannot start another instance.\n");
        return -FILE_ERROR;
    }
    /* 操作成功 */
    return FILE_EOK;
 ...
 *            将文件截断到指定长度 __len，并设置文件偏移到 __ofs。
 * 
 * @param[in] __pf   文件控制结构体指针（必须已打开）
 * @param[in] __len  目标截断长度（字节），不能为负
 * @param[in] __ofs  截断后希望设置的文件偏移量
 * @param[in] __cmd  截断方式：
 *                 - FILE_F_TRUNCATE ：对已打开文件描述符使用 ftruncate
 *                 - FILE_TRUNCATE   ：对指定路径使用 truncate（路径通过结构体成员传入）
 * 
 * @retval    FILE_EOK    成功
 * @retval    FILE_ERROR  失败（参数错误或系统调用失败）
 * 
 * @note      成功后 __pf->ret 记录截断前后的大小差异（单位：字节），并打印文件信息。
 */
int _file_truncate(_file_t *__pf ,off_t __len ,off_t __ofs ,int __cmd)
{
    /* 参数校验：文件指针不能为空，长度不能为负，截断命令必须有效 */
    if(__pf==NULL || __len<0 ||
       (__cmd!=FILE_F_TRUNCATE && __cmd!=FILE_TRUNCATE))
        return -FILE_ERROR;
    ...
    /* 保存文件截断前大小 */
    off_t fs = __pf->fst->st.st_size;
    int ret = 0;

    /* 根据截断命令选择调用方式 */
    if(__cmd==FILE_F_TRUNCATE)
    {
        /* 通过文件描述符截断文件长度 */
        ret = ftruncate(__pf->fd ,__len);
    }
    else if(__cmd==FILE_TRUNCATE)
    {
        /* 检查文件是否存在 */
        if(__ACCESS_MODE(__pf->__pathname ,F_OK)==-1)
            return -FILE_ERROR;

        /* 通过路径截断文件长度 */
        ret = truncate(__pf->__pathname ,__len);
    }
    ...
    /* 设置文件偏移量，失败返回错误 */
    if(_file_set_offset(__pf ,__ofs ,SEEK_SET)==-FILE_ERROR)
        return -FILE_ERROR;

    /* 更新文件信息，失败返回错误 */
    if(_file_get_info(__pf)==-FILE_ERROR)
        return -FILE_ERROR;

    /* 计算并保存文件大小变化（绝对值差异） */
    __pf->ret = (__len > fs) ? (__len - fs) : (fs - __len);

#ifdef PRINT
    /* 打印文件信息（调试用） */
    PRINT_FILE_INFO("truncate" ,__pf);
#endif
