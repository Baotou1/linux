...
#define __UMASK(__mode)\
                        do{\
                            ...
                            fprintf(stdout ,"[UMASK] Changed umask: %04o → %04o\n", __old, __mode);\
                        }while(0)
/**
 * @macro    __ACCESS
...
#define __ACCESS_MODE(__pathname, __mode)\
                                         ((access((__pathname), (__mode)) != -1) ? 0 : (-1))
...
/**
 * @macro  CLOSE_FILE_FD
 * @brief  关闭当前进程中打开的所有文件描述符（包括标准输入/输出/错误）
 *
 * @details
 *  此宏用于清理进程中已打开的文件描述符，常用于守护进程化（daemon）或子进程中防止文件泄露。
 *  通过 sysconf(_SC_OPEN_MAX) 获取系统允许的最大 FD 数，遍历并逐一关闭。
 *  如果无法获取最大 FD 数，使用默认值 1024。
 *
 * @note
 *  执行后，标准输入（0）、标准输出（1）、标准错误（2）也会被关闭，需谨慎使用。
 */
#define CLOSE_ALL_FDS\
                    do{\
                        long __max_fd = sysconf(_SC_OPEN_MAX);      \
                        if (__max_fd == -1)                         \
                            __max_fd = 1024;  /* 默认最大 FD 数 */    \
                        for (int __i = 0; __i < __max_fd; ++__i)    \
                            close(__i);                             \
                    }while(0)
/******************************************************************************************************************/
/**
