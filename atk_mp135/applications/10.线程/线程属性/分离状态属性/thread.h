...
/**
 * @enum thread_op_t
 * @brief  线程创建操作类型标志，用于控制线程初始化时的行为
 *
 * @details
 * 枚举 thread_op_t 表示线程创建时的附加操作选项，可用于控制是否启用实时调度策略、
 * 是否设置线程为分离状态等行为。用于 __thd_t 结构体中的 __op 字段，由上层调用逻辑决定。
 */
typedef enum 
{
    THREAD_OP_DEFAULT        = 0,        ///< 0b00：默认操作，使用系统默认属性
    THREAD_OP_REALTIME       = (1 << 0), ///< 0b01：启用实时调度，设置调度策略和优先级
    THREAD_OP_DETACHED       = (1 << 1)  ///< 0b10：设置线程为分离状态（detached）
    // 可拓展：如绑定CPU (1 << 2)、自定义栈大小等
} thread_op_t;
