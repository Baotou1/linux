 ...
 */
struct _list_head{
    struct _list_head *__next;  ///< 指向链表中下一个节点的指针，形成链表连接
};
 ...
 *
 * @details
 *  该结构体用于实现双向循环链表中的节点链接部分，
 *  包含指向前一个节点和后一个节点的指针，实现链表的双向连接。
 *
 * 通过将该结构体嵌入到包含数据的结构体中，可以方便地管理链表关系。
 *
 * @note
 *  - __next 指向链表中的下一个节点
 *  - __prev 指向链表中的上一个节点
 *  - 链表的头节点的 __next 和 __prev 指针指向自身，表示空链表
 */
struct _dlist_head{
    struct _dlist_head *__next; ///< 指向链表中下一个节点的指针
    struct _dlist_head *__prev; ///< 指向链表中上一个节点的指针
};
typedef struct _dlist_head _dlist_h;
