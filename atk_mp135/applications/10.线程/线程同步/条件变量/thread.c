    ...
    ,void *__data ,int __num)
{
    ...
    /* 使用已初始化的属性对象初始化互斥锁 */
    __ret = pthread_mutex_init(&__mutex->__obj, &__mutex->__attr);
...
int __sync_mutex_trylock(__sync_mutex_t *__mutex)
{
    ...
    return pthread_mutex_trylock(&__mutex->__obj);
}
...
int __sync_mutex_lock(__sync_mutex_t *__mutex)
{
    ...
    return pthread_mutex_lock(&__mutex->__obj);
}
...
int __sync_mutex_unlock(__sync_mutex_t *__mutex)
{
    ...
    return pthread_mutex_unlock(&__mutex->__obj);
}
/**
...
int __sync_mutex_destroy(__sync_mutex_t *__mutex)
{
    ...
    /* 先销毁互斥锁本身 */
    if(pthread_mutex_destroy(&__mutex->__obj) != 0)
    {
        return -1;
    }
...
int __sync_cond_wait(__sync_cond_t *__cond)
{
    ...
    return pthread_cond_wait(&__cond->__obj, &__cond->__mutex.__obj);
}
