...
{
    int __pashared = PTHREAD_PROCESS_PRIVATE;
    if(__sync_rwlock_init(&__rwlock ,&__pashared ,&__count ,1) != 0)
    {
        PROCESS_EXIT_FLUSH(&__proc, -1);
    }
}
...
void thread_init(void)
{
    ...
    /* 填充主线程信息 */
    __proc->__pthdl->__pthd = __thread_init("main", NULL ,NULL);
    ...
    /* 写加锁 */
    __thd_t *__pthd_1= __thread_init("thd1", __thread_1 ,NULL);
    __thd_t *__pthd_2= __thread_init("thd2", __thread_1 ,NULL);
    __thd_t *__pthd_3= __thread_init("thd3", __thread_1 ,NULL);
    __thd_list_add_nd(__proc->__pthdl, __pthd_1);
    __thd_list_add_nd(__proc->__pthdl, __pthd_2);
    __thd_list_add_nd(__proc->__pthdl, __pthd_3);

    /* 读加锁 */
    __thd_t *__pthd_4= __thread_init("thd4", __thread_2 ,NULL);
    __thd_t *__pthd_5= __thread_init("thd5", __thread_2 ,NULL);
    __thd_t *__pthd_6= __thread_init("thd6", __thread_2 ,NULL);
    __thd_list_add_nd(__proc->__pthdl, __pthd_4);
    __thd_list_add_nd(__proc->__pthdl, __pthd_5);
    __thd_list_add_nd(__proc->__pthdl, __pthd_6);

    __thread_create(__pthd_1);
    __thread_create(__pthd_2);
    __thread_create(__pthd_3);
    __thread_create(__pthd_4);
    __thread_create(__pthd_5);
    __thread_create(__pthd_6);
#endif
}
...
void thread_exit_handler(void *__arg)
{
    ...
    /* 退出线程，释放链表节点，返回退出值（存于 __ret） */
    __thread_exit(__proc, __pthd, __pthd->__ret);
}
