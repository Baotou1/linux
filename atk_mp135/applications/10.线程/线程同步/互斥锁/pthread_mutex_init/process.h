 ...
 *
 * @details
 *  该结构体封装了进程的各种关键信息，便于对进程进行统一管理、调度和控制。
 *  成员涵盖进程标识、子进程跟踪、信号处理、exec 上下文、命令、线程与文件管理等，
 *  为用户态构建自定义进程抽象模型提供基础支持。
 *  
 *  可广泛应用于简易 shell 实现、自定义进程控制框架、嵌入式调度器等场景。
 */
struct __proc_struct
{
    char *__name;            ///< 进程逻辑名称，便于日志记录和调试，不同于系统级进程名
    pid_t __pid;             ///< 进程 ID，由 fork 产生，唯一标识该进程
    pid_t __ppid;            ///< 父进程 ID，标识创建当前进程的父进程
    pid_t __pgid;            ///< 进程组 ID，用于将多个进程分组调度（如 job 控制）
    pid_t __sid;             ///< 会话 ID（Session ID），通常等于创建者 PID，用于终端控制

    __cproc_t __cproc;       ///< 子进程管理信息，记录子进程 PID、数量、状态等
    __sig_t *__sig;          ///< 指向信号处理结构体，支持信号处理函数绑定与屏蔽设置
    __exec_t __exec;         ///< exec 执行上下文，保存执行路径、参数列表、环境变量指针

    char *__command;         ///< 原始命令字符串，保存用户输入的命令，常用于日志或 system 实现
    __flist_t *__pfl;        ///< 文件资源链表头，管理进程打开的文件（可封装 open/close 逻辑）
    __tlist_t *__pthdl;      ///< 线程链表头指针，挂载该进程所属的线程列表（用于主线程+子线程管理）
};

typedef struct __proc_struct __proc_t;
extern __proc_t *__proc;
/**
                                                    ...
                                                    do{\
                                                        ...
                                                        {\
                                                            ...
                                                            if((*__proc)->__pthdl != NULL)\
                                                                __thd_list_free(&(*__proc)->__pthdl);\
                                                            __proc_free(__proc);\
                                                        }\
                                                        __exit_fn(__ret);\
                                                    }while(0)
