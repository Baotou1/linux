...
void __proc_exit(void)
{
    ...
    fprintf(stdout ,"mutex_lock[0]_type = %d\n" ,__sync_get_mutexattr(&lock[0]));
    int __type = PTHREAD_MUTEX_NORMAL;
    __sync_set_mutexattr(&lock[0] ,__type);
    fprintf(stdout ,"mutex_lock[0]_type = %d\n" ,__sync_get_mutexattr(&lock[0]));
    fprintf(stdout ,"time = %f\n" ,(__tim2 - __tim1));
    LOG_PRINT("INFO", __proc, NULL, "exit %s process ,pid=%lu",
        __proc->__name,
        __proc->__pid);
    __sync_mutex_destroy(&lock[0]);
...
int main(int argc, char *argv[])
{
    ...
    /* 初始化数据锁 */
    int __type = PTHREAD_MUTEX_ERRORCHECK;
    __sync_mutex_init(&lock[0] , &__type, &g_count ,1);
