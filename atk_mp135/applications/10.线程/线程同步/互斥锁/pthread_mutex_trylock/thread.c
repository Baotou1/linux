 ...
 *  - 该函数直接调用 pthread_mutexattr_gettype 来获取属性
 */
int __sync_get_mutexattr(__sync_mutex_t *__sync)
{
    ...
    int __ret = pthread_mutexattr_gettype(&__sync->__mutexattr, &__type);
    if (__ret != 0)
        /* 获取失败，返回错误码 -1 */
        return -1;
 ...
 *
 * @param __sync  指向同步锁结构体的指针，不能为空
 * @param __type  要设置的互斥锁类型，如 PTHREAD_MUTEX_NORMAL、PTHREAD_MUTEX_RECURSIVE 等
 *
 * @return
 *  - 成功返回 0；
 *  - 失败返回非零错误码；
...
int __sync_set_mutexattr(__sync_mutex_t *__sync ,int __type)
{
    ...
    /* 调用 pthread_mutexattr_gettype 获取锁类型 */
    return pthread_mutexattr_settype(&__sync->__mutexattr, __type);
}
    ...
    ,void *__data ,int __num)
{
    ...
    /* 初始化互斥锁属性对象 */
    int __ret = pthread_mutexattr_init(&__sync->__mutexattr);
    if(__ret != 0){
        /* 初始化失败，重置结构体状态 */
        __sync->__data = NULL;
        __sync->__num  = 0;
    ...
    if(__type != NULL)
    {
        __ret = pthread_mutexattr_settype(&__sync->__mutexattr ,*__type);
        if(__ret != 0){
            /* 设置类型失败，销毁属性对象，重置状态 */
            pthread_mutexattr_destroy(&__sync->__mutexattr);
            __sync->__data = NULL;
            __sync->__num  = 0;
            return __ret; 
        }
    ...
    /* 根据属性初始化互斥锁，如果 __type 为 NULL 则使用默认属性 */
    __ret = pthread_mutex_init(&__sync->__mutex, &__sync->__mutexattr);
 ...
 * @brief 非阻塞尝试加锁
 *
 * @param __sync   同步结构体指针，不能为空
 *
 * @retval 0       成功获得锁
 * @retval -1      参数非法（__sync 为 NULL）
...
int __sync_mutex_trylock(__sync_mutex_t *__sync)
{
    ...
    return pthread_mutex_trylock(&__sync->__mutex);
}
...
int __sync_mutex_destroy(__sync_mutex_t *__mutex)
{
    ...
    /* 先销毁锁属性 */
    if(pthread_mutexattr_destroy(&__mutex->__mutexattr) != 0)
    {
        return -1;
    }

    /* 再销毁互斥锁本身 */
    if(pthread_mutex_destroy(&__mutex->__mutex) != 0)
    {
        return -1;
    }
