...
int _sfile_fwrite(_sfile_t *psfw ,const void *ptr ,size_t sz, size_t nmemb)
{
    ...
    if(_file_data_init(&psfw->ptr ,sz * nmemb) == -1)
        return -FILE_ERROR;

    printf("%s file write..." ,psfw->name);
    ...
    psfw->fsz = _file_get_size(psfw->pf->_fileno);
...
int _sfile_fread(_sfile_t *psfr ,size_t sz, size_t nmemb)
{
    ...
    if(psfr->ptr != NULL){
        free(psfr->ptr);
        psfr->ptr = (void*)calloc((sz * nmemb) ,sizeof(char));
        if(psfr->ptr == NULL){
            free(psfr->ptr);
            printf("%s fread calloc errpr...\n" ,psfr->name);
        }
    }
    fread(psfr->ptr ,sz ,nmemb ,psfr->pf);
