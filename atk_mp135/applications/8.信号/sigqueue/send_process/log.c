 ...
 * @brief   写入日志信息到日志文件。
 * 
 * @param[in]   __data_str  要写入的日志信息字符串，不能为空。
 * @return      成功返回写入的字节数，失败返回 -1。
 * 
 * @details
 *  本函数会获取当前时间戳，将时间戳和日志信息拼接成一行完整字符串，
 *  格式为“时间戳 + 空格 + 日志信息”，然后追加写入日志文件。
 *  若输入参数为空或时间戳获取失败，函数返回 -1。
 */
int _log_write(const char *__pr_n_str, const char *__fmt_str, ...)
{
    if(__log == NULL || __pr_n_str == NULL || __fmt_str == NULL 
                                                        || __ACCESS_MODE(LOGFILE ,F_OK) == -1)
        return -1;
    ...
    char __msg_buf[192];  /* 格式化后的日志内容 */
    va_list args;
    va_start(args, __fmt_str);
    vsnprintf(__msg_buf, sizeof(__msg_buf), __fmt_str, args);
    va_end(args);

    /* 构造完整字符串：时间戳 + 空格 + 名字 + 数据 + 换行 */   
    snprintf(__log->__str, 256 ,"%s %s %s\n" ,__tim_str ,__pr_n_str ,__msg_buf);
