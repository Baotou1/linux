 ...
 */
int _log_write(const char *__name_str ,const char *__data_str)
{
    if(__log == NULL || __name_str == NULL || __data_str == NULL || __ACCESS_MODE(LOGFILE ,F_OK) == -1)
        return -1;
    ...
    snprintf(__log->__str, 256 ,"%s %s %s\n" ,__tim_str ,__name_str ,__data_str);
