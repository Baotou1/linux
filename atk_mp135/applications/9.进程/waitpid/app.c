...
int main(int argc ,char *argv[])
{
    ...
    for(int i = 0 ;i < 3 ;i++)
    {
        ...
        switch(__proc->__cproc.__pid[i])
        {
            ...
            case 0:
                ...
                sleep(i);/* 控制子进程结束时间,子进程睡眠时间为：0秒、1秒、2秒，然后退出 */
                PROCESS_EXIT_FAST(&__proc ,i);
            default:
    ...
    printf("~~~~~~~~~~~~~~\n");
    PROCESS_REFRESH_INFO("NULL" ,__proc);
    int __cnt = __proc->__cproc.__cnt;
    for(;;)
    {
        pid_t __ret = 0;
        __ret = __proc_waitpid(-1 ,&__proc->__cproc.__sta ,0); 
        if(__ret == -1){
            PROCESS_EXIT_FLUSH(&__proc ,-1);
        }
        else if(__ret == 0)
        ...
        else
        {
            ...
            for(i = 0 ;i < __cnt ;i++)
            {
                if(__proc->__cproc.__pid[i] == __ret)
                {
                    break;
                }
            }
