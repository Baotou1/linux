...
/**
 * @func   __proc_wait
 * @brief  等待任意一个子进程结束，并返回其进程 ID。
 *
 * @param[in,out] __sta  指向一个整数的指针，用于保存子进程的退出状态信息。
 *                       成功时，该变量将包含由 wait() 设置的退出状态。
 *
 * @retval pid_t         成功时返回已结束子进程的 PID；
 *                       若无子进程可等待且 errno == ECHILD，则返回 -1；
 *                       若发生其他错误，也返回 -1，并调用 PRINT_ERROR() 打印错误信息。
 *
 * @details
 *  封装系统调用 wait() 的辅助函数，用于等待任意一个子进程终止。
 *  - 如果参数 __sta 为 NULL，则直接返回 -1；
 *  - 调用 wait() 等待一个子进程终止，返回其 PID；
 *  - 若 wait() 返回 -1 且 errno != ECHILD，表示出现非“无子进程”类错误，调用 PRINT_ERROR() 输出错误信息；
 *  - 若 errno 为 ECHILD，说明当前进程无子进程可等待，不视为严重错误，直接返回 -1。
 *
 * @note
 *  此函数建议用于多进程程序中子进程的统一回收处理，可结合 WIFEXITED 和 WEXITSTATUS 宏进一步分析退出状态。
 *
 * @example
 *  int status;
 *  pid_t pid = __proc_wait(&status);
 *  if (pid > 0) {
