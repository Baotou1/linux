...
/**
 * @name    __proc_setsid
 * @brief   创建新的会话（session），使当前进程成为会话首领
 *
 * @retval pid_t
 *      成功时返回新会话的 ID（即当前进程的 PID）；
 *      失败返回 -1，并打印错误信息（errno 指示原因）
 *
 * @details
 *  本函数封装了 setsid() 系统调用，用于将当前进程设置为新会话的会话首领（Session Leader），
 *  同时脱离其当前进程组和控制终端。该操作通常用于守护进程（daemon）创建流程。
 *
 * @note
 *  - 若当前进程已经是一个进程组的组长（PGID == PID），调用 setsid() 会失败，并返回 -1；
 *  - 失败时 errno 可能为：
 *      - EPERM：当前进程已经是进程组组长，不能创建新会话；
 *  - 一旦 setsid() 成功，新的会话 ID 等于当前进程的 PID；
 *  - 成为会话首领后，进程将失去对原控制终端的关联（若有）；
 *  - 守护进程通常在 setsid() 之后关闭标准输入/输出，并切换工作目录；
 *
 * @see getsid(2), setsid(2)
 */
pid_t __proc_setsid(void)
{
    if(setsid() == -1)
    {
        PRINT_ERROR();
