...
/**
 * @func   __proc_wait
 * @brief  等待任意一个子进程结束，并返回其进程 PID。
 *
 * @param[out] __sta 指向一个整数的指针，用于接收子进程的退出状态。
 *                   成功时，该变量将由 wait() 设置退出信息。
 *
 * @retval pid_t     成功时返回已结束子进程的 PID；
 *                   若无子进程可等待（errno == ECHILD）或发生其他错误，返回 -1。
 *
 * @details
 *  此函数封装了系统调用 wait()，用于阻塞等待任意一个子进程结束。
 *  - 若传入的 __sta 为 NULL，则直接返回 -1；
 *  - 若 wait() 返回 -1 且 errno == ECHILD，表示当前进程没有可等待的子进程；
 *  - 若 errno 为其他错误类型，则调用 PRINT_ERROR() 输出详细信息；
 *  - 若等待成功，返回终止子进程的 PID，并在 __sta 中提供其退出信息。
 *
 * @note
 *  可结合 WIFEXITED(status)、WEXITSTATUS(status) 等宏分析子进程退出原因。
 *  本函数适用于简单子进程的统一回收场景。
 */
 ...
 *
 * @details
 *  此函数封装系统调用 waitpid()，可等待特定子进程或任意子进程退出。
 *  - 若 __sta 为 NULL，立即返回 -1；
 *  - 若 waitpid() 调用失败且 errno != ECHILD，则调用 PRINT_ERROR()；
 *  - 若 errno == ECHILD，表示当前进程无可等待子进程，不视为严重错误；
 *  - 成功时，返回终止的子进程 PID，并在 __sta 中提供其退出状态。
 *
 * @note
 *  使用 WNOHANG 可实现非阻塞等待，若没有退出的子进程，将返回 0。
 *  建议结合 WIFEXITED/WEXITSTATUS 宏分析子进程退出情况。
 */
