...
#define PRINT_DIRFILE_INFO(action, pdf)\
                            printf(\
                                "[File Info]\n"                                               \
                                "├─ File Name    : %s\n"                                      \
                                "├─ Action       : %s\n"                                      \
...
typedef struct {
    char *__pathname;                 /**< 文件或目录的绝对路径 */
    struct __file_stat *__fst;       /**< 指向文件属性信息结构体的指针（如 stat 信息） */
    DIR *__dirp;                     /**< 目录流指针，用于目录遍历的 opendir 返回值 */
    struct dirent **__dirs;          /**< 指向当前读取到的目录项信息 */
    int __counts;
} _dfile_t;


_dfile_t* _dfile_init(char *__pathname);
int _dfile_open(_dfile_t *__pdf);
int _dfile_close(DIR *__dir);
int _dfile_mkdir(char *__pathname ,mode_t __md);
int _dfile_rmdir(char *__pathname);
#ifdef __cplusplus
}
#endif
