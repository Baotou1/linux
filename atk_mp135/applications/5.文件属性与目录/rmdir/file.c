...
int _dfile_read(DIR *__dirp ,struct dirent **__dir ,int *__count)
{
    if(__dirp == NULL)
        return -FILE_ERROR;

    errno = 0;
    *__dir = readdir(__dirp);
    
    if(*__dir == NULL)
        return (errno != 0) ? -FILE_ERROR : FILE_EEOF; 
...
int _dfile_allread(_dfile_t *__pdf)
{
    ...
    int i = 0;
    _dfile_read(__pdf->__dirp ,&__pdf->__dirs[i] ,&__pdf->__counts);
}
/**
 * @name   _dfile_mkdir
 * @brief  创建目录：
